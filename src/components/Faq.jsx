                    ...
                    <div className="max-w-[735px] gap-6 flex flex-col mx-auto">
                        {FAQS_LIST.map((item, index) => (
                            <div className={`relative pl-10 pr-[30px] max-sm:px-4 py-[15px] transition-all duration-300 ease-linear ${active === index ? "!py-8" : ""}`}  key={index}>
                                <Image className='pointer-events-none absolute left-0 top-0 max-w-[735px] w-full h-full' src="/assets/images/faq-bg.webp" alt='faq bg' width={735} height={71}/>
                                <button onClick={() => toggle(index)} className='relative uppercase z-[1] leading-[202%] items-center cursor-pointer flex w-full text-black justify-between max-sm:gap-1 bg-transparent text-left text-base max-sm:text-[9px]' >
                                    {item}{" "}
                                ...
                                <p className={`relative z-[1] text-sm leading-[202%] text-black overflow-hidden max-[992px]:overflow-y-auto transition-all max-sm:text-[8px] duration-500 max-w-[992px] ${active === index ? "max-h-32 max-sm:max-h-52 pt-1" : "max-h-0"}`} >
                                We want to avoid overpromising from the get go. For that, we are approaching roadmaps slightly differently.
                                </p>
                            </div>
                        ))}
                    </div>
